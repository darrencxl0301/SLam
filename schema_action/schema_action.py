...
# ⭐ PATCH: Force UTF-8 encoding for all file operations
...
def _patched_read_excel(*args, **kwargs):
    # Excel files don't use encoding parameter, but this ensures consistency
...
class MultiTableActionLibrary:
    ...
    def _load_file_data(self, file_path: str) -> pd.DataFrame:
        ...
        df = pd.read_excel(file_path, encoding='utf-8') if file_path.endswith(('.xlsx', '.xls')) else pd.read_csv(file_path, encoding='utf-8')
...
class GenericActionLibrary:
    ...
    def _load_file_data(self, file_path: str) -> Dict[str, pd.DataFrame]:
        ...
        if file_type == 'excel':
            ...
            data = {"Sheet1": self._clean_dataframe(pd.read_csv(file_path, encoding='utf-8'))}
